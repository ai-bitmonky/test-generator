...
async function enrichExistingQuestions() {
  ...
  console.log('   • Solution verification (matches correct answer)');
    ...
    try {
      ...
      if (missingOptions && q.solution_html && q.correct_answer) {
        ...
        try {
          ...
          console.log(`            Option A: ${generatedOptions.option_a.substring(0, 50)}...`);
          console.log(`            Option B: ${generatedOptions.option_b.substring(0, 50)}...`);
          console.log(`            Option C: ${generatedOptions.option_c.substring(0, 50)}...`);
          console.log(`            Option D: ${generatedOptions.option_d.substring(0, 50)}...`);

          // Update the question object and database immediately
      ...
      if (!completeness.isComplete || completeness.isAmbiguous) {
        ...
        // Continue with enrichment, but flag issue
      } else {
        console.log('      ✅ Question complete');
      }
      await new Promise(resolve => setTimeout(resolve, CONFIG.baseDelay));

      // 3. Verify solution matches correct answer
